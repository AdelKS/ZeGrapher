...
scripts/.
  ...
  ../icons/ZeGrapher.svg   the app icon, copied to <out>/assets/images/
    icons/*.svg            one standalone SVG per icon, written in where used
  content/<lang>/*.md      the panels of one language
  content/<lang>/strings.yaml  the short strings of that language: the words of
                           the footer, and the words that ask for a donation
  content/<lang>/images/   the pictures those panels show
  download/                the bundles of one release, which the download
                           tiles link to
...
The title of a panel is the '# ' heading that its markdown file opens with,
translated like the rest of the file. A file that opens with anything else stops
the build.
...
that each entry opens. The 'footer:' mapping of content/<lang>/strings.yaml
holds the words of each key, in that language:

  footer:
    donate: Faire un don

The words of an entry are the text of the link. If the entry also carries a
'link-name', that name becomes the link, and the words stand in front of it as
plain text. The credit of the theme reads that way: 'Based on Astral by HTML5
UP'.

The copyright is the one entry that neither file holds. It opens every strip,
and it comes from the COPYRIGHT of this script.
...
part-translated language still holds every panel. A key that the strings file
leaves out falls back to English too.
...
A front matter holds the strings that a language translates and that no heading
carries. Only the landing panel has one:

  language     the name of this language, written in that language. The
               drop-down list at the top of every page shows it, and a language
               folder needs it
...
running ZeGrapher reads one of the two to see whether a new release is out.
'latest_tag' holds the tag, and 'latest' holds the same version as a decimal
number, which ZeGrapher 3 reads.

An empty download/, or no download/ at all, stops the build. Pass --no-bundles
to build a page whose tiles link the releases page on GitHub. Use it when you
have no bundle at hand: it writes no 'latest' file, so it is not a build to
deploy. A folder that holds only a part of a release stops the build, and so
does a bundle of an alpha, a beta or a release candidate. The site carries
releases only.
...
# The drawing of the app icon, which the site shows as its logo and its favicon.
# One file holds it, and the build copies it into the site, so no second copy of
# the drawing lives in the repository.
APP_ICON = ROOT / "icons" / "ZeGrapher.svg"

# The line that opens the footer. It holds no word to translate, so it sits here
# and not in build.yaml. The year of the build closes the range. The two
# '&nbsp;' keep the gap after the year: a plain space would collapse into one.
COPYRIGHT = "© 2011-{year} &nbsp;&nbsp; Adel Kara Slimane"

# Every page is written to '<out>/<lang>/index.html', so the root of the site is
# one folder above the page. Every path in a page starts there.
SITE_ROOT = ".."
...
# The bundles that the tiles link to, as (platform, architecture). Every tile
# shows one platform, so a build carries all four of them, or none at all.
...
def read_conf() -> dict:
    ...
    before pandoc runs.
    """
    conf = content.read_conf()
    for field in ("icon-map", "gallery", "hero-image", "footer"):
        if not conf.get(field):
            sys.exit(f"{CONF_FILE}: '{field}' holds nothing")

    if not conf["footer"].get("links"):
        sys.exit(f"{CONF_FILE}: 'footer: links' holds nothing, and it names every "
...
def read_languages(landing: dict) -> dict:
    ...
    content.languages() finds the languages. The 'language' field of the front
    matter of the landing panel gives the name of each one, written in that
    language. The drop-down list at the top of every page shows it.
    """
    names = {}
    for lang in content.languages():
        ...
        if not front.get("language"):
            sys.exit(f"{md}: the front matter holds no 'language'. It gives the "
                     f"name of this language, written in that language, such "
                     f"as 'language: Français'")
...
def panel_source(panel: dict, lang: str) -> Path:
    ...
    return content.file_of(lang, panel["file"])


def picture_path(name: str, lang: str) -> str:
    """The path of one picture, as a panel writes it, checked against content/.

    copy_pictures() writes both languages into the same output folder, so one
    path covers the translated picture and the English one behind it.
    """
    if name in content.pictures(lang):
        return f"{PICTURES}/{name}"
    holders = " nor ".join(f"content/{folder}/{PICTURES}/"
                           for folder in dict.fromkeys((lang, SOURCE)))
...
def copy_pictures(lang: str, out_root: Path) -> None:
    ...
    for name, path in content.pictures(lang).items():
...
def localize_pictures(html: str, lang: str, md: Path) -> str:
    ...
    def replace(match: "re.Match") -> str:
        ...
        if not content.file_of(lang, url).is_file():
...
def resolve_doc_links(html: str, md: Path, panel_ids: dict) -> str:
    ...
    A doc links to another doc by file name. Here every doc is a panel of the
    same page, so the link becomes an anchor. The heading slug gets the prefix
    of the target panel, the same one pandoc put on the id of that heading.
    """
    def replace(match: "re.Match") -> str:
        prefix, url, suffix = match.groups()
...
def check_portable(md: Path, text: str) -> None:
    """Reject what only pandoc can render, in the files that the app shares.

    The app renders the manual with the markdown importer of Qt, which accepts
    a small part of the HTML that pandoc accepts. A raw HTML block that looks
    right on the site then degrades in the doc viewer, and nothing reports it.
    Use a generated block of this build when markdown cannot write the markup.
    """
    # the whole file is scanned, and the fenced code is blanked out instead of
    # removed, so the line number reported is the one an editor shows
...
def read_bundles(folder: Path, no_bundles: bool) -> tuple[str, dict] | None:
    ...
    A folder that is not there, or that holds no file, stops the build. With
    'no_bundles' both cases return None, and the tiles then link GitHub.
...
def copy_bundles(bundles: dict, version: str, out_root: Path) -> dict:
    ...
    'ZeGrapher-<os>-<arch>-<version>' and the extension of the file it was
    copied from. The deploy replaces the site with this build, so 'download/'
    holds the bundles of this release alone. GitHub keeps every older release.
    """
    links = {}
...
def downloads_html(release: dict | None) -> str:
    ...
    version line is written. This is the build to use when you have no bundle
    at hand.
    """
    if release is None:
...
def read_strings(lang: str) -> dict:
    """The strings file of one language, empty when that language has none."""
    path = CONTENT / lang / STRINGS
    if not path.is_file():
        return {}
    strings = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    if not isinstance(strings, dict):
        sys.exit(f"{path}: the file holds no mapping of names to strings")
    return strings


def donation_words(lang: str) -> tuple[str, Path]:
    """The markdown that asks for a donation, English when a language has none."""
    for folder in dict.fromkeys((lang, SOURCE)):
        words = read_strings(folder).get("donation")
        if words:
            return words, CONTENT / folder / STRINGS
    sys.exit(f"{CONTENT / SOURCE / STRINGS}: it holds no 'donation', and the "
             f"'{{{{ donation }}}}' block shows those words")


def donation_html(conf: dict, lang: str, panel_id: str) -> str:
    """The words that ask for a donation, and one big icon under them.
    ...
    """
    url = content.donate_url(conf)

    text, path = donation_words(lang)
    check_portable(path, text)
...
def decimal_version(version: str) -> str:
    ...
    ZeGrapher 3 compares this file with the number it was built with, and
    'v3.1.1' was built as 3.11. The first dot is the decimal point, and the dots
    after it are dropped: '4.0.1' becomes 4.01, which is more than every 3.x
    number. A version that carries a word, such as '4.0.0_beta1', loses that
    word, so the build reads as the release it comes before.
    """
...
def write_version_files(release: dict | None, out_root: Path) -> None:
    ...
    them. 'latest_tag' holds the tag of the release, which ZeGrapher 4 reads in
    src/Utils/updatecheck.cpp and compares with the version it was built from.
    'latest' holds the same version as a decimal number, which ZeGrapher 3
    reads.

    A build that holds no release writes neither file, so it cannot publish a
    version over the one the site already holds.
    """
    if release is None:
        return
...
def footer_words(lang: str, links: dict) -> dict:
    """The words of the footer of one language, out of its strings file.

    Each name under 'footer:' there is an entry of 'footer: links' in
    build.yaml, and its value is the words that entry shows.
    """
    words = read_strings(lang).get("footer") or {}
    unknown = sorted(set(words) - set(links))
    if unknown:
        sys.exit(f"{CONTENT / lang / STRINGS}: it holds the words of "
                 f"{', '.join(unknown)}, and the 'footer: links' of {CONF_FILE} "
                 f"names no such entry")
    return words
...
def footer_html(footer: dict, lang: str) -> str:
    ...
    the order of the file. The words of an entry come from the strings file of
    this language, and from the English one for a key that language leaves out.

    The words of an entry are the text of the link. If the entry also carries a
    'link-name', that name becomes the link, and the words stand in front of it
    as plain text. A name such as 'Astral by HTML5 UP' is not translated.
    """
    links = footer["links"]
    words = footer_words(SOURCE, links)
    missing = sorted(set(links) - set(words))
    if missing:
        sys.exit(f"{CONTENT / SOURCE / STRINGS}: it holds the words of no "
...
def htaccess(template: str, languages: list) -> str:
    ...
    Apache reads one alternation of language names, so a language the site does
    not hold gets no rule. A browser that asks for such a language lands on the
    English page, instead of a folder that does not exist.
    """
...
def title_of(md: Path, body: str):
    ...
    title of the panel, translated like the rest of the file, and the panel
    shows it. markdown_to_html() drops it to the <h2> of the panel.
    """
    lines = body.lstrip("\n").split("\n")
...
def build_language(lang: str, site: Site) -> Path:
    ...
    for i, panel in enumerate(site.panels):
        ...
        if "donation" in names:
            blocks["donation"] = donation_html(site.conf, lang, meta["id"])
...
def main() -> None:
    ...
    ap.add_argument("--no-bundles", action="store_true",
                    help="build a page whose tiles link the releases page on "
                         ...
                         "is a build to look at, not one to deploy")
    args = ap.parse_args()
    ...
    if not APP_ICON.is_file():
        sys.exit(f"{APP_ICON} does not exist, and it is the logo of the site")
    logo = args.out / "assets" / "images" / APP_ICON.name
    logo.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(APP_ICON, logo)
