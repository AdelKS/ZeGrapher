...
# the suffixes of the documents an override is merged into. An override
# replaces every other document whole
MERGED = {".zg", ".yaml"}
...
class Patches:
    ...
    def use(self, patch):
        ...
        The captures are grouped by patch, so each patch is applied one time.
        """
...
def shoot(document, output):
    ...
    try:
        ...
        # last-document.zg with the document we opened
        app.kill()
        app.wait()
...
def capture_all(captures, langs):
    ...
    # the base ones, so the sources stay as they were. The next run builds the
    # sources without the patch again
    try:
        # the group without a patch captures the binary of the build directory,
        # so it is built here: Patches.use never builds for that group
        build()
        # Patches.use prints the header of every patched group. The group without
        # a patch comes first, and it prints its header only when it has a
        # capture left to take
        plain = False
...
def cut(shot, rect, output):
    ...
    # optimize picks the zlib filter that writes the smallest file. The call
    # names every metadata chunk that goes out, and nothing else is written
