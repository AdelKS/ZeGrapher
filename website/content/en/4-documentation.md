...
- [7. The About tab](#7-the-about-tab)
...
| **App** | language, font, syntax colors |
| **About** | version, updates, licenses, release notes |

The three buttons at the bottom left are for [files](#22-files).

On the edge of the panel:

- the arrow button folds the panel away, and brings it back
- the bookmark button under it opens and closes this documentation
- drag the line along the right edge to change the width of the panel

### 2.1. View range

The four fields at the top of the panel accept expressions, not just numbers. A
minimum must stay below its maximum, and the field turns invalid if it does
not.
...
- Drag the handle on the right to reorder the objects.
...
own, with a name and the same buttons as any other object.
...
columns to a CSV file, and add a column. Drag the bar underneath to resize the
sheet, and double-click it to give the sheet its default height again.
...
- Press `Backspace` to clear the content of the selected cells.
- Press `Delete` to remove the selected cells. The values below them move up.
- The right-click menu holds those same two actions, under *Clear* and
  *Delete*, and it adds *Insert row above* and *Insert row below*. Both Insert
  entries act on the active cell.

Selecting a whole column and pressing `Delete` also deletes the column object
in the Math tab.
...
It opens a file dialog. A side pane then shows the import settings, with a
preview of the file underneath. The preview updates as you change a setting:

- **Separator** — the character between two values. Write `\t` for a `TAB`.
- **Header row** — check it when the first row holds the column names.
- **Preview rows** — how many rows the preview shows.
- **Skip rows** — how many rows to ignore at the top of the file, such as
  comments or extra parameters.

**Load** makes columns out of the file, and **Cancel** leaves everything as it
was.
...
**Scaling**, in the same box, resizes everything the app draws at once: the
lines, the text and the ticks.

The app draws that sheet as a page in the window, with a zoom bar above it:

![The zoom bar](images/zoom-bar.png)

The bar zooms the sheet on the screen, and the last button fits the whole sheet
in the window. This zoom does not touch the graph: the bounds of the view stay
as they are. Centimeter sizes are correct at 100% zoom.

The two boxes under **Size**:

- **Drawing**: the font and the background color of the graph. **Min points**
  and **Max points** set how many points the app computes for each continuous
  curve, as a power of two. More points give a finer curve and a slower redraw.
...
This tab sets the language and the font of the interface, and the three colors
of the input lines: valid, warning and invalid.

## 7. The About tab

This tab tells what ZeGrapher is. It holds:

- the version you run, and a button that asks zegrapher.com whether a newer one
  is out,
- links to the website, to the source code and to the author,
- the licenses of ZeGrapher, of Qt and of the bundled math font,
- the release notes: what each release brought, with a link to its page on
  GitHub.
