...
- [7. Вкладка «О программе»](#7-вкладка-о-программе)
...
Четыре поля наверху панели — это границы вида. Под ними пять вкладок:
...
| **Общие** | язык, шрифт, цвета подсветки |
| **О программе** | версия, обновления, лицензии, список изменений |

Три кнопки внизу слева — для [файлов](#22-файлы).

На краю панели:

- кнопка со стрелкой сворачивает панель и разворачивает её обратно
- кнопка-закладка под ней открывает и закрывает эту документацию
- потяните полосу вдоль правого края, чтобы изменить ширину панели

### 2.1. Границы вида

Четыре поля наверху панели принимают выражения, а не только числа. Каждый
минимум должен оставаться меньше своего максимума, иначе поле станет
неверным.
...
- Потяните ручку справа, чтобы изменить порядок объектов.
...
математический объект: у него есть имя и те же кнопки, что и у любого другого
объекта.
...
добавляют столбец. Потяните полосу под ними, чтобы изменить высоту листа, а
двойной щелчок по ней вернёт листу высоту по умолчанию.
...
- Нажмите `Backspace`, чтобы очистить содержимое выделенных ячеек.
- Нажмите `Delete`, чтобы удалить выделенные ячейки. Значения под ними
  поднимутся.
- Те же два действия есть в меню по правой кнопке, под *Очистить* и *Удалить*, а
  вместе с ними *Вставить строку выше* и *Вставить строку ниже*. Обе вставки
  работают от текущей ячейки.

Если выделить весь столбец и нажать `Delete`, объект-столбец на вкладке
«Объекты» удалится вместе с ним.
...
Она открывает выбор файла. Затем сбоку появляются настройки загрузки, а под
ними — просмотр файла. Просмотр обновляется при каждой смене настройки:

- **Разделитель** — знак между двумя значениями. Для табуляции напишите `\t`.
- **Строка заголовков** — отметьте её, если в первой строке стоят имена
  столбцов.
- **Показать строк** — сколько строк видно в просмотре.
- **Пропустить строк** — сколько строк пропустить в начале файла, например
  комментарии или параметры.

**Загрузить** создаёт столбцы по файлу, а **Отмена** оставляет всё как было.
...
**Масштаб**, в той же рамке, разом меняет размер всего, что рисуется: линий,
текста и делений.
...
Полоса меняет масштаб листа на экране, а последняя кнопка вписывает лист
целиком в окно. Этот масштаб не трогает сам график: границы вида остаются
прежними. Размеры в сантиметрах верны при масштабе 100 %.

Две рамки под **Размером**:

- **Отрисовка**: шрифт графика и цвет его фона. **Точек мин** и **Точек макс**
  задают, сколько точек считается для каждой непрерывной кривой, в степенях
...
На этой вкладке задаются язык и шрифт интерфейса, а также три цвета полей
ввода: верно, внимание и неверно.

## 7. Вкладка «О программе»

Эта вкладка рассказывает, что такое ZeGrapher. На ней:

- версия, которой вы пользуетесь, и кнопка, которая спрашивает у zegrapher.com,
  не вышла ли новее,
- ссылки на сайт, на исходный код и на автора,
- лицензии ZeGrapher, Qt и встроенного математического шрифта,
- список изменений: что принёс каждый выпуск, со ссылкой на его страницу на
  GitHub.
