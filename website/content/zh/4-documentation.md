...
- 点击表头可选中整行或整列。
- 按住并拖动可选中一块矩形区域。
- 按 `Backspace` 清空选中单元格里的内容。
- 按 `Delete` 删除选中的单元格，下方的值会往上移。
- 右键菜单里同样有这两个操作，对应*清除*和*删除*，
  另外还有*在上方插入行*和*在下方插入行*。两个插入操作都作用于当前单元格。

选中一整列再按 `Delete`，数学标签页里对应的列对象也会一起删除。
...
它会打开文件选择框。随后侧栏显示导入设置，下面是文件的预览。每改一项设置，
预览就跟着更新：

- **分隔符**——两个值之间的字符。制表符写 `\t`。
- **标题行**——第一行是列名时勾选它。
- **预览行数**——预览显示多少行。
- **跳过行数**——文件开头要跳过多少行，比如注释或额外参数。

**加载**按文件的内容建出各列，**取消**则什么都不改。
...
同一个框里的**缩放**会一次改变所画一切的大小：线条、文字和刻度。

应用把这张纸当作窗口里的一页来画，上方带一条缩放条：

![缩放条](images/zoom-bar.png)

这条缩放条在屏幕上缩放这张纸，最后一个按钮让整张纸正好装进窗口。
这个缩放不影响图本身：视图的边界保持不变。按厘米给出的尺寸在 100% 缩放下才准确。

**尺寸**下面的两个框：

- **绘制**：图的字体和背景颜色。**最少点数**和**最多点数**决定应用为每条
  连续曲线计算多少个点，按 2 的幂给出。点越多，曲线越细腻，画得也越慢。
...
这个标签页设置界面的语言和字体，以及输入框的三种颜色：有效、警告和无效。

## 7. 关于标签页

这个标签页介绍 ZeGrapher。上面有：

- 你正在使用的版本，以及一个按钮，用来问 zegrapher.com 有没有更新的版本；
- 指向网站、源代码和作者的链接；
- ZeGrapher、Qt 和内置数学字体的许可证；
- 发行说明：每个版本带来了什么，并附有该版本在 GitHub 上的页面链接。
