...
- [7. L'onglet À propos](#7-longlet-à-propos)
...
| **Appli** | langue, police, coloration syntaxique |
| **À propos** | version, mises à jour, licences, notes de version |

Les trois boutons en bas à gauche concernent les [fichiers](#22-fichiers).

Sur le bord du panneau :

- le bouton flèche replie le panneau, et le ramène
- le bouton marque-page, juste en dessous, ouvre et ferme cette documentation
- faites glisser la ligne qui longe le bord droit pour changer la largeur du
  panneau

### 2.1. Bornes de la vue

Les quatre champs en haut du panneau acceptent des expressions, et pas seulement
des nombres. Chaque minimum doit rester inférieur à son maximum, sinon le champ
devient invalide.
...
- Faites glisser la poignée à droite pour réordonner les objets.
...
objet mathématique à part entière, avec un nom et les mêmes boutons que
...
les colonnes vers un fichier CSV, et ajoutent une colonne. Faites glisser la
barre en dessous pour redimensionner la feuille, et double-cliquez dessus pour
lui rendre sa hauteur par défaut.
...
- Appuyez sur `Retour arrière` pour effacer le contenu des cellules
  sélectionnées.
- Appuyez sur `Suppr` pour supprimer les cellules sélectionnées. Les valeurs qui
  se trouvent en dessous remontent.
- Le menu contextuel, d'un clic droit, reprend ces deux actions, sous *Effacer*
  et *Supprimer*, et y ajoute *Insérer une ligne au-dessus* et *Insérer une
  ligne en dessous*. Les deux entrées d'insertion agissent sur la cellule
  active.

Sélectionner toute une colonne puis appuyer sur `Suppr` supprime aussi l'objet
colonne dans l'onglet Maths.
...
Il ouvre un sélecteur de fichier. Un volet latéral montre ensuite les réglages
d'importation, avec un aperçu du fichier en dessous. L'aperçu se met à jour à
chaque réglage modifié :

- **Séparateur** — le caractère entre deux valeurs. Écrivez `\t` pour une
  tabulation.
- **Ligne d'en-tête** — cochez-la quand la première ligne contient les noms des
  colonnes.
- **Lignes à afficher** — le nombre de lignes que l'aperçu montre.
- **Lignes à ignorer** — le nombre de lignes à sauter en haut du fichier, des
  commentaires ou des paramètres par exemple.

**Charger** crée les colonnes à partir du fichier, et **Annuler** laisse tout en
l'état.
...
et le reste dans un `pdf` ou un `svg` exporté.

**Échelle**, dans le même cadre, redimensionne d'un coup tout ce que
l'application dessine : les traits, le texte et les graduations.
...
La barre zoome l'affichage de la feuille, et le dernier bouton ajuste toute la
feuille à la fenêtre. Ce zoom ne touche pas au graphe : les bornes de la vue
restent inchangées. Les tailles en centimètres sont justes à 100 % de zoom.

Les deux cadres sous **Taille** :

- **Dessin** : la police et la couleur d'arrière-plan du graphe. **Points min**
  et **Points max** fixent le nombre de points que l'application calcule pour
  chaque courbe continue, en puissance de deux. Plus il y a de points, plus le
  tracé est fin et plus le dessin est lent.
...
Cet onglet règle la langue et la police de l'interface, ainsi que les trois
couleurs des champs de saisie : valide, avertissement et invalide.

## 7. L'onglet À propos

Cet onglet dit ce qu'est ZeGrapher. On y trouve :

- la version que vous utilisez, et un bouton qui demande à zegrapher.com s'il en
  existe une plus récente,
- des liens vers le site, vers le code source et vers l'auteur,
- les licences de ZeGrapher, de Qt et de la police mathématique embarquée,
- les notes de version : ce qu'apporte chaque version, avec un lien vers sa page
  sur GitHub.
