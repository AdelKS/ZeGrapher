...
パネル上部の四つの入力欄が表示範囲です。その下に五つのタブがあります：
...
| **グラフ** | 大きさ、フォント、背景、軸 |
| **アプリ** | 言語、フォント、構文の色分け |
| **情報** | バージョン、更新、ライセンス、変更点 |

左下の三つのボタンは[ファイル](#22-ファイル)用です。

パネルの端には：

- 矢印のボタン。パネルをたたみ、もう一度押すと開きます
- そのすぐ下のしおりボタン。このドキュメントを開いたり閉じたりします
- 右端の線。ドラッグするとパネルの幅が変わります

### 2.1. 表示範囲

パネル上部の四つの入力欄は、数だけでなく式も受け付けます。最小値は必ず最大値より
小さくなければならず、そうでないとその欄は無効になります。
...
データの対象は、名前のついた列を集めたシートです。列は一つひとつが数学の対象で、
名前を持ち、ほかの対象と同じボタンが並びます。
...
列の追加です。その下の棒をドラッグするとシートの高さが変わり、ダブルクリック
すると既定の高さに戻ります。
...
- ドラッグすると四角い範囲を選べます。
- `Backspace` で選んだセルの中身を消します。
- `Delete` で選んだセルそのものを消します。下にある値が上に詰まります。
- 右クリックのメニューにも同じ二つが*消去*と*削除*として並び、さらに
  *上に行を挿入*と*下に行を挿入*があります。挿入はどちらも、いま選んでいる
  セルを基準にします。

列をまるごと選んで `Delete` を押すと、数式タブにあるその列の対象も消えます。
...
ファイル選択の画面が開きます。そのあと横のパネルに読み込みの設定が出て、その下に
ファイルのプレビューが並びます。設定を変えるたびにプレビューが変わります：

- **区切り文字**——値と値の間の文字。タブは `\t` と書きます。
- **見出し行**——一行目が列の名前のときにチェックします。
- **表示行数**——プレビューに出る行の数。
- **読み飛ばす行数**——ファイルの先頭で読み飛ばす行の数。コメントや
  パラメーターなど。

**読み込む**でファイルから列を作り、**キャンセル**なら何も変わりません。
...
同じ枠の**拡大率**は、描かれるもの全体の大きさを一度に変えます。線も、文字も、
目盛りもまとめてです。
...
バーは画面上で紙をズームし、いちばん右のボタンは紙全体をウィンドウに収めます。
このズームはグラフには触れません。表示範囲はそのままです。センチメートルで
指定した大きさが正しくなるのは、ズーム 100% のときです。

**サイズ**の下の二つの枠：

- **描画**：グラフのフォントと背景の色。**最小点数**と**最大点数**は、連続な
  曲線一本あたりに計算する点の数を決めます。2 のべき乗で指定します。点が多い
  ほど曲線は滑らかになり、描くのは遅くなります。
...
このタブでは画面の言語とフォント、そして入力欄の三つの色（有効、警告、無効）を
決めます。

## 7. 情報タブ

このタブは ZeGrapher が何かを伝えます。ここには：

- 使っているバージョンと、新しい版が出ていないか zegrapher.com に問い合わせる
  ボタン、
- サイト、ソースコード、作者へのリンク、
- ZeGrapher、Qt、そして同梱の数式フォントのライセンス、
- 変更点：各バージョンで何が変わったか。GitHub のページへのリンク付きです。
