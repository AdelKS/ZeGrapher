...
- [7. La pestaña Acerca de](#7-la-pestaña-acerca-de)
...
| **App** | idioma, fuente, colores de sintaxis |
| **Acerca de** | versión, actualizaciones, licencias, notas de versión |

Los tres botones de abajo a la izquierda son para los [archivos](#22-archivos).

En el borde del panel:

- el botón de la flecha pliega el panel, y lo vuelve a traer
- el botón del marcapáginas, justo debajo, abre y cierra esta documentación
- arrastra la línea que recorre el borde derecho para cambiar el ancho del
  panel

### 2.1. Límites de la vista

Los cuatro campos de arriba del panel aceptan expresiones, no solo números. Cada
mínimo debe quedar por debajo de su máximo, y si no lo hace el campo pasa a no
válido.
...
- Arrastra el asa de la derecha para reordenar los objetos.
...
matemático de pleno derecho, con nombre y los mismos botones que cualquier otro
objeto.
...
las columnas a un archivo CSV, y añaden una columna. Arrastra la barra de debajo
para cambiar la altura de la hoja, y haz doble clic en ella para devolverle su
altura por defecto.
...
- Pulsa `Retroceso` para borrar el contenido de las celdas seleccionadas.
- Pulsa `Supr` para eliminar las celdas seleccionadas. Los valores que están
  debajo suben.
- El menú contextual, con el botón derecho, tiene esas dos mismas acciones, bajo
  *Borrar* y *Eliminar*, y añade *Insertar fila encima* e *Insertar fila
  debajo*. Las dos entradas de inserción actúan sobre la celda activa.

Seleccionar una columna entera y pulsar `Supr` borra también el objeto columna
de la pestaña Mates.
...
Abre un selector de archivos. Después, un panel lateral muestra los ajustes de
importación, con una vista previa del archivo debajo. La vista previa se
actualiza con cada ajuste que cambias:

- **Separador** — el carácter entre dos valores. Escribe `\t` para el tabulador.
- **Fila de cabecera** — márcala cuando la primera fila lleva los nombres de las
  columnas.
- **Mostrar filas** — cuántas filas enseña la vista previa.
- **Omitir filas** — cuántas filas hay que saltarse al principio del archivo,
  comentarios o parámetros por ejemplo.

**Cargar** crea las columnas a partir del archivo, y **Cancelar** lo deja todo
como estaba.
...
sigue siendo en un `pdf` o un `svg` exportado.

**Escala**, en el mismo recuadro, cambia de una vez el tamaño de todo lo que
dibuja la aplicación: las líneas, el texto y las marcas.
...
La barra hace zoom sobre la hoja en la pantalla, y el último botón ajusta la
hoja entera a la ventana. Este zoom no toca la gráfica: los límites de la vista
no se mueven. Los tamaños en centímetros son correctos al 100 % de zoom.

Los dos recuadros bajo **Tamaño**:

- **Dibujo**: la fuente y el color de fondo de la gráfica. **Puntos mín** y
  **Puntos máx** fijan cuántos puntos calcula la aplicación para cada curva
  continua, en potencias de dos. Cuantos más puntos, más fino el trazado y más
  lento el dibujo.
...
Esta pestaña ajusta el idioma y la fuente de la interfaz, y los tres colores de
los campos de entrada: válida, aviso y no válida.

## 7. La pestaña Acerca de

Esta pestaña cuenta qué es ZeGrapher. Tiene:

- la versión que usas, y un botón que le pregunta a zegrapher.com si hay una más
  nueva,
- enlaces al sitio, al código fuente y al autor,
- las licencias de ZeGrapher, de Qt y de la fuente matemática incluida,
- las notas de versión: lo que trae cada versión, con un enlace a su página en
  GitHub.
