...
int main()
{
  ...
  // a build between two releases reads as the release it comes before
  test(parse_version_string("4.0.0_rc0-dev") == rc);

  // the pattern is anchored: a string that holds a version somewhere in it is
  // no version
  test(not parse_version_string("ZeGrapher 4.0.0").has_value());
  test(not parse_version_string("4.0.0 and more").has_value());
  test(not parse_version_string("v4.0.0_gamma1").has_value());
