...
def version_key(tag: str) -> tuple[int, ...] | None:
    ...
    src/Utils/versionparser.h orders a version the same way, and the app orders
    the releases with it. It reads two forms that no tag carries: the '-r<n>' of
    a continuous build, and the '-dev' of a tree between two releases.
    """
    match = TAG.match(tag)
    if match is None:
        return None
